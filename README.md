...
통계 그룹 기능을 배포하기 전에는 이어서
`supabase/migrations/202609110009_statistics_groups.sql`을 한 번 실행해야 합니다.
...
`설정`의 `통계 그룹 관리`에서 수입 그룹과 지출 그룹을 따로 만듭니다. 상세 분류는 한 번에
하나의 그룹에만 속하며, 다른 그룹에서 선택해 저장하면 새 그룹으로 이동합니다. 그룹을 삭제해도
상세 분류와 거래는 삭제되지 않고 그룹 연결만 해제됩니다. 가계부 입력은 계속 상세 분류를 선택해
기록하며, 통계 그룹은 통계 화면의 묶음 표시용으로만 사용합니다.
...
만들고 거래·통계·설정·프로필·비밀번호·공동 장부·세금·휴지통 정보를 변경하는 E2E는 통계 그룹
마이그레이션을 포함한 전용 개발 Supabase에서만
아래 값을 추가해 실행하세요.
...
통계 그룹 E2E는 세 상세 분류와 거래를 만든 뒤 그룹 합계·펼치기·다른 그룹으로의 이동·그룹
삭제 후 단독 분류 복귀를 확인하고, 공동 장부 일반 구성원에게 변경 버튼이 없는지도 확인합니다.
모든 생성 데이터는 전용 개발 프로젝트의 안전 표시를 다시 확인한 뒤 테스트 계정 삭제로 정리합니다.
