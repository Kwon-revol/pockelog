...
export function LedgerScreen({ initialData, createAction, updateAction, trashAction, loadPage }: LedgerScreenProps) {
  ...
  return (
    <div className="space-y-7">
      ...
      <form action="/ledger" className="grid grid-cols-1 gap-3 rounded-3xl border border-slate-200/80 bg-white p-4 shadow-sm md:grid-cols-2 lg:grid-cols-7" method="get">
        <label className="block min-w-0 text-xs font-bold text-slate-500 lg:col-span-1">시작일<span className="relative mt-1.5 block min-w-0"><input className="block w-full min-w-0 max-w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" defaultValue={initialData.filters.startOn} name="start" type="date" /></span></label>
        <label className="block min-w-0 text-xs font-bold text-slate-500 lg:col-span-1">종료일<span className="relative mt-1.5 block min-w-0"><input className="block w-full min-w-0 max-w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" defaultValue={initialData.filters.endOn} name="end" type="date" /></span></label>
        <label className="block min-w-0 text-xs font-bold text-slate-500 md:col-span-2 lg:col-span-2">검색<input className="mt-1.5 block w-full min-w-0 max-w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" defaultValue={initialData.filters.query} maxLength={100} name="q" placeholder="내용 또는 메모" /></label>
        <label className="block min-w-0 text-xs font-bold text-slate-500">유형<select className="mt-1.5 block w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm" defaultValue={initialData.filters.type} name="type"><option value="all">전체</option><option value="expense">지출</option><option value="income">수입</option></select></label>
        <label className="block min-w-0 text-xs font-bold text-slate-500">분류<select className="mt-1.5 block w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm" defaultValue={initialData.filters.categoryId ?? ""} name="category"><option value="">전체</option>{initialData.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
